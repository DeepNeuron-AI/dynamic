...
success_counter = 0
for (LV_masks_list, filenames), (RV_masks_list, filenames) in zip(run_inference(dataloader), run_inference(flipped_dataloader)):
    for LV_masks, RV_masks, filename in zip(LV_masks_list, RV_masks_list, filenames):
        ...
        if filename == '0X100F044876B98F90.avi':
            np.save(filename, LV_masks)
            import sys
            sys.exit(0)
