...
# Might help to think about what the deployment context looks like - Jon is talking a lot about cloud infrastructure?
# Can we really do sequential integration of data, one at a time?
# Further fine tuning on new datasets, weighting _against_ the data we've already trained the AI on?
...
def run(
    ...
    dataloader = torch.utils.data.DataLoader(
                    ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"), drop_last=True)
    valloader = torch.utils.data.DataLoader(
                    valset, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"), drop_last=False)
